...
export function FloatingDockClient({ navItems }: FloatingDockClientProps) {
  ...
  const links: DockLink[] = useMemo(() => {
    const base: DockLink[] = navItems.map((item) => ({
      title: item.title || "",
      href: item.href || "#",
    ...
    return base;
  }, [navItems, isSignedIn, isSidebarOpen, signOut]);
  
  const desktop = useMemo(
    () => getVisibleLinks(links, MAX_VISIBLE_ITEMS_DESKTOP),
    [links]
  );
  const mobile = useMemo(
    () => getVisibleLinks(links, MAX_VISIBLE_ITEMS_MOBILE),
    [links]
  );
  ...
  return (
    <>
      ...
      <div
        className={`hidden md:block fixed z-30 transition-all duration-300 group/dock ${
          isSidebarOpen
